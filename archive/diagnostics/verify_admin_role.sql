...
BEGIN
    ...
    -- Verify no company associations
    IF EXISTS (SELECT 1 FROM user_companies WHERE user_id = admin_user_id) THEN
        RAISE EXCEPTION 'Admin user should not have any company associations';
    END IF;
    
    IF EXISTS (SELECT 1 FROM profiles WHERE id = admin_user_id AND current_company_id IS NOT NULL) THEN
        RAISE EXCEPTION 'Admin user profile should not have a current_company_id';
    END IF;
END $$; 